        ...
        <div class="card">
            <div class="header">
                ...
                <nav class="pull-right">
                    {!! link_to_route('admin.blog.create', 'Tambah Article',$parameters = array() , array('class'=>'btn btn-primary btn-block')); !!}
                </nav>
            </div>
                ...
                <table class="table table-hover table-striped">
                    ...
                    <tbody>
                        @foreach($blog as $data)
                          {!! Form::open(array('class' => 'form-inline', 'method' => 'DELETE', 'route' => array('admin.blog.destroy', $data->id))) !!}
                        <tr>
                            <td>{{$x++}}</td>
                            <td>{{substr($data->title, 0,50)}}</td>
                            <td>{!! substr($data->article, 0,160) !!}</td>
                            <td>
                                <a href="{{ url('admin/blog') }}/{{ $data->id }}" class="btn" >
                                    <i class="pe-7s-look " style="font-size: 20px; color:#4AA3DF;"></i>
                                </a>
                                <a href="{{ url('admin/blog') }}/{{ $data->id }}/edit" class="btn" >
